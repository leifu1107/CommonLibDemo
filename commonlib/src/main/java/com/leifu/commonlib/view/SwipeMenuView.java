        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    Log.e(TAG, "dispatchTouchEvent " + mViewCache);
                    if (mViewCache != null) {
                        if (mViewCache != this) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (!iosInterceptFlag) {//且滑动了 才判断是否要收起、展开menu
                        ...
                        if (Math.abs(velocityY) < Math.abs(velocityX) && Math.abs(velocityX) > 1000) {//滑动速度超过阈值
                            if (velocityX < -1000) {
                                if (isLeftSwipe) {//左滑
                                    //平滑展开Menu
                                    Log.e(TAG, iosInterceptFlag + " " + isLeftSwipe + " " + isLeftSwipe);
                                    smoothExpand();
                                } else {
                                    //平滑关闭Menu
                                    smoothClose();
...
//            //模仿IOS 点击其他区域关闭：
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                //add by 2016 09 11 ，IOS模式开启的话，且当前有侧滑菜单的View，且不是自己的，就该拦截事件咯。滑动也不该出现
                float gap = mLastP.y - ev.getRawY();
                //为了在水平滑动中禁止父类ListView等再竖直滑动
                if (Math.abs(gap) > 10 || Math.abs(getScrollY()) > 10) {//2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
                    getParent().requestDisallowInterceptTouchEvent(true);
                    return true;
                }
